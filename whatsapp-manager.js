...
class WhatsAppManager {
    ...
    async autoRestoreSessions() {
        console.log('🔄 Checking for sessions to auto-restore from database...');
        ...
        try {
            ...
            console.log(`🗄️ [LocalAuth] Found ${savedSessions.length} session(s)`);

            if (savedSessions.length === 0) {
                console.log('📭 No saved sessions in database');
                await ProfileLogger.autoRestore(null, 'no_sessions');
                return;
            }

            console.log(`🗄️ Found ${savedSessions.length} session(s) in database`);
            ...
            for (const session of savedSessions) {
                ...
                if (match) {
                    ...
                    if (profile) {
                        ...
                        console.log(`⚠️ Session exists for profile ${profileId} but profile not in database, skipping...`);
            ...
            if (profilesToRestore.length === 0) {
                console.log('📭 No valid profiles to restore');
                await ProfileLogger.autoRestore(null, 'no_sessions', { reason: 'no_valid_profiles' });
                return;
            }

            console.log(`📱 Restoring ${profilesToRestore.length} profile(s): ${profilesToRestore.map(p => p.device_name || p.id).join(', ')}`);

            for (const profile of profilesToRestore) {
                // Skip if already connected in this session
                if (this.clients.has(profile.id)) {
                    console.log(`⏭️ Profile ${profile.id} (${profile.device_name}) already has active client, skipping...`);
                    await ProfileLogger.autoRestore(profile.id, 'skipped', { reason: 'already_connected' });
                    continue;
                }

                console.log(`🔌 Auto-restoring profile ${profile.id} (${profile.device_name})...`);
                ...
                try {
                    ...
                    console.error(`❌ Failed to auto-restore profile ${profile.id}:`, error.message);
    ...
    async createClient(profileId, retryCount = 0) {
        ...
        if (this.clients.has(profileId)) {
            ...
            if (existing.status === 'error') {
                console.log(`⚠️ Cleaning up failed client for profile ${profileId}...`);
                ...
                console.log(`⚠️ Client already exists for profile ${profileId}`);
                return existing;
            }
        }

        console.log(`🚀 Creating WhatsApp client for profile ${profileId}...${retryCount > 0 ? ` (retry ${retryCount})` : ''}`);
        ...
        console.log(`🗄️ Using LocalAuth for profile ${profileId}`);
        ...
        console.log(`🚀 Launching Puppeteer browser for profile ${profileId}...`);
        ...
        try {
            ...
            console.error(`❌ Failed to initialize client for profile ${profileId}:`, error.message);
            ...
            if (retryCount < MAX_RETRIES) {
                console.log(`🔄 Retrying initialization for profile ${profileId}...`);
                await new Promise(resolve => setTimeout(resolve, 2000));
    ...
    _setupEventHandlers(profileId, client) {
        ...
        client.on('authenticated', async () => {
            ...
            try {
                ...
                console.log(`📊 Database updated: Profile ${profileId} → connected`);
            } catch (dbError) {
                console.error(`❌ DB update error for profile ${profileId}:`, dbError.message);
            }
        ...
        client.on('ready', async () => {
            console.log(`✅ Profile ${profileId} is ready!`);
        ...
        client.on('disconnected', async (reason) => {
            console.log(`❌ Profile ${profileId} disconnected:`, reason);
        ...
        client.on('auth_failure', async (message) => {
            console.error(`❌ Profile ${profileId} auth failure:`, message);
        ...
        client.on('loading_screen', (percent, message) => {
            console.log(`⏳ Profile ${profileId} loading: ${percent}% - ${message}`);
        ...
        client.on('message_ack', async (msg, ack) => {
            ...
            */
            ...
            try {
                ...
                if (ack >= 2 && msg.fromMe) {
                    console.log(`🔍 ACK received: ack=${ack}, messageId=${messageId}, profileId=${profileId}`);
                }
                ...
                if (newStatus && msg.fromMe) {
                    ...
                    try {
                        ...
                        if (campaignId) {
                            console.log(`📊 Campaign message ${messageId} status updated to: ${newStatus}`);
                        ...
                        console.log(`⚠️ Campaign ACK check: ${campaignError.message}`);
                    }
                }
            } catch (error) {
        ...
        client.on('message', async (msg) => {
            try {
                ...
                console.error(`❌ Critical error in message handler for profile ${profileId}:`, error.message);
        ...
        try {
            ...
            if (clientData && clientData.connectedAt) {
                ...
                if (messageTimestamp < oldMessageThreshold) {
                    ...
                    console.log(`⏭️ Skipping old message from ${msg.from} (received while offline)`);
                    return;
                }
            }
            ...
            if (profile?.webhook_url) {
                ...
                console.log(`📤 Webhook sent for profile ${profileId}`);
            ...
            if (!msg.fromMe && msg.body) {
                ...
                if (autoReply) {
                    if (client) {
                        ...
                        console.log(`🤖 Auto-reply sent for profile ${profileId}`);
                        // Log auto-reply
    ...
    async sendMessage(profileId, chatId, message) {
        ...
        console.log(`📤 Message sent via profile ${profileId}`);
